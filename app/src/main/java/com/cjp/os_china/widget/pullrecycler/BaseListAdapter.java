...
public abstract class BaseListAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    protected static final int VIEW_TYPE_HEADER_VIEW = 101;
    protected boolean isLoadMoreFooterShown;
    protected boolean isHeaderViewShown;
    protected BaseViewHolder mHeaderViewHolder;

    @Override
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_LOAD_MORE_FOOTER) {
            return onCreateLoadMoreFooterViewHolder(parent);
        } else if (viewType == VIEW_TYPE_HEADER_VIEW) {
    ...
    public int getItemCount() {
        return getDataCount() + (isLoadMoreFooterShown ? 1 : 0) + (isHeaderViewShown ? 1 : 0);
    }
    ...
    public int getItemViewType(int position) {
        ...
        } else if (isHeaderViewShown && position == 0) {
